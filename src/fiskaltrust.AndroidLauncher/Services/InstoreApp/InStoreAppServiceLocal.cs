...
public sealed class InStoreAppServiceLocal : IInStoreAppService, IDisposable
{
    ...
    public async Task<(PayResponse? response, PaymentErrorResponse? errorResponse)> TriggerPaymentAsync(InStoreAppOptions options, PayRequest request)
    {
        ...
        payRequests[options.OperationId] = (tcsAcknowledge, tcsPayResponse);

        try
        {
            await EnsureConnectedAsync().ConfigureAwait(false);
            await PublishPayRequestAsync(options.OperationId, options.CashBoxId, options.AccessToken, request).ConfigureAwait(false);
        ...
        {
            payRequests.TryRemove(options.OperationId, out _);
        }
    }
